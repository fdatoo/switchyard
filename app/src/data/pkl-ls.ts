...
const SVC = "switchyard.pkllsp.v1.PklLsService";
...
export async function definition(
  req: { filePath: string; source: string; line: number; col: number },
  opts: RpcOptions = {},
): Promise<{ filePath: string; line: number; col: number }> {
  const res = await rpcCall<
    unknown,
    { file_path?: string; filePath?: string; line?: number; col?: number }
  >(`${SVC}/Definition`, { filePath: req.filePath, source: req.source, line: req.line, col: req.col }, opts);
  return {
...
export async function semanticTokens(
  req: { filePath: string; source: string },
  opts: RpcOptions = {},
): Promise<{ data: Uint32Array }> {
  const res = await rpcCall<unknown, { data?: number[] }>(
    `${SVC}/SemanticTokens`,
    { filePath: req.filePath, source: req.source },
    opts,
  );
