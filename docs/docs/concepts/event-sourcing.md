...
!!! status-alpha "Alpha — shipped, interface evolving"
    The event store is shipped and the core behaviour described here is stable. The `switchyard events` CLI surface may change before v1.0.
...
switchyard works differently. Every state change — a light turns on, a door opens, an automation fires, a command is sent — is written as an **event** appended to an immutable log. Nothing is ever updated or deleted. Current state is computed from the log. "What is the kitchen light right now?" is answered by replaying the events that affected it and returning the final value.

This design is called **event sourcing**.

The event log is stored in SQLite, inside the switchyard data directory. It grows over time. The daemon reads it on startup, loads the most recent snapshot, replays events from there, and arrives at current state. All of this happens before the daemon starts accepting connections.
...
```
switchyard events replay --at "2026-01-15T02:00:00"
```
...
```
switchyard events replay --at "2026-01-15T02:00:00" --entity light.kitchen_ceiling
```
...
```
switchyard events list --since "2026-01-15T00:00:00" --until "2026-01-15T06:00:00"
```
...
If `switchyardd` crashes or is restarted, it does not lose any state. It loads the nearest snapshot, replays events forward from that point, and arrives at exactly the correct current state before accepting connections. No stale cache, no inconsistency.

### Commands are in the log too

When switchyard sends a command to a driver — `turn_on`, `set_temperature` — a `CommandIssued` event is written. When the driver responds, a `CommandAcknowledged` or `CommandFailed` event is written. This means you can answer questions like: "Did my midnight lights-off automation actually reach every light, or did some commands time out?"

```
switchyard events list \
...
You wake up and find the living room lights are on. You check the switchyard events log to figure out what happened.
...
The light turned on at 02:12:07. The source is `driver:hue_main`, meaning the Hue driver reported the change — something physically happened (a switch, an app, or a Hue routine), not an automation in switchyard.

**Step 2: Confirm no switchyard automation was involved.**
...
No automation fired in that window. No commands were issued by switchyard. The Hue driver simply reported a state change that originated on the Hue side.

**Step 3: If you want to see what state everything was in at that moment.**

```
switchyard events replay --at "2026-01-15T02:12:06"
```
...
**Conclusion:** The lights came on because someone (or something) changed the light through the Hue app or a physical switch, not because of a switchyard automation. The switchyard event log let you reach that conclusion in under two minutes without guessing.

## What it costs

### Disk space

The event log grows over time. switchyard writes an event only when entity state actually changes — not on every poll cycle. A driver may poll a device every 30 seconds, but if the device hasn't changed state, no event is written.
...
By default, switchyard keeps everything indefinitely. You can configure per-kind retention in Pkl:
...
On startup, switchyardd loads the nearest snapshot and replays events forward. At typical scale (200 entities, a few hundred MB to 2 GB/year), this takes a few seconds. Snapshots are taken every 10,000 events or every hour (whichever comes first), so the replay window is bounded.

If you have a very old installation with hundreds of millions of events and no recent snapshot, startup can be slower. The `switchyard snapshot create` command forces a new snapshot to be written, capping future replay windows.
...
This is what makes switchyard's state trustworthy: there is one authoritative record, it never goes away, and everything else is derived from it.
