...
This page is for people who have outgrown HA's model and want to understand what switchyard does differently, what it deliberately keeps from HA, and what you give up by switching.

## What switchyard fixes

These are design decisions in HA that switchyard makes differently. In most cases the HA approach reflects reasonable constraints from when that design was established; switchyard is newer and can make different tradeoffs.

| Home Assistant | switchyard |
|---|---|
...
The driver/driver-instance split is the most structural difference. In HA, "integration" conflates the code (the Python module) with the config (the credentials and options you enter). In switchyard, a driver is just a binary; a driver instance is a Pkl declaration that binds that binary to specific parameters. You can run three Hue instances against three bridges using one driver binary. The concepts are cleanly separate.
...
## What switchyard keeps from HA

switchyard deliberately preserves the parts of HA's model where familiarity helps and the model is sound. If you have used HA, these will feel familiar:
...
- **Areas** — spatial groupings of entities. The concept is the same; switchyard adds hierarchy.
...
- **Scripts** — callable named procedures. In switchyard they are Starlark functions registered under a name with typed parameters.
...
This list is honest. These are real limitations of switchyard v1.0.

**No HA API compatibility shim.** switchyard does not implement the HA REST or WebSocket API. Anything that talks to HA as a backend — HA's mobile app, third-party dashboards that use the HA API, HA Cloud services — will not work with switchyard without modification. This is explicitly deferred and may be addressed in a later version if there is sufficient demand, but it is not planned for v1.0.

**HACS integrations do not transfer.** HACS (Home Assistant Community Store) custom integrations are Python code that runs inside HA's runtime. They have no equivalent in switchyard. If you depend on a HACS integration for a device that has no first-party switchyard driver, you cannot use switchyard for that device yet.

**Supervisor add-ons do not transfer.** HA's Supervisor add-on ecosystem runs containers managed by the HA OS. switchyard has no equivalent. If you use Supervisor add-ons (Zigbee2MQTT-as-add-on, for example), those run independently of switchyard — which is actually fine, because switchyard can talk to Zigbee2MQTT over MQTT or the Z2M bridge driver.

**The HA mobile app will not work unmodified.** The HA Companion app is built against the HA API. It will not connect to switchyard. switchyard's web UI is a PWA installable on mobile; a native wrapper is deferred.

**Smaller ecosystem at launch.** HA has been developed since 2013 with thousands of contributors. switchyard has a fraction of that history and a fraction of the driver coverage. First-party drivers (v0.1.0 alpha ships: MQTT, Zigbee2MQTT bridge, Hue, ESPHome native, generic REST, generic webhook; v1.0 target adds: Matter, HomeKit bridge, Z-Wave JS bridge, Nest). Devices outside that set require a custom driver or waiting for the ecosystem to grow.
