...
If you are running Home Assistant today and want to move to gohome, this section is your guide. Before reading further, see [gohome vs. Home Assistant](../introduction/vs-home-assistant.md) — it explains what the two systems do differently and what you give up by switching. Come back here when you have decided to migrate.

---

## Overview

`gohome import-ha` is a CLI command that reads your Home Assistant configuration directory and produces a fresh gohome Pkl config tree. The output is a git-initable directory you review, edit, and commit before pointing `gohomed` at it. The importer is a one-shot tool: run it, inspect the result, resolve any issues it flags, then proceed with your gohome setup.
...
- **gohome installed.** The importer ships as part of the `gohome` CLI. See [Installation](../installation/index.md).
...
3. **Maps** each recognised HA integration to the corresponding gohome driver instance declaration. Integrations outside gohome's v1.0 driver set are preserved as labelled placeholders in the output so you know what manual work remains.
...
5. **Translates** the area registry, entity registry, zones, scenes, users, and persons into their gohome Pkl equivalents.
6. **Writes** a complete, git-initable output directory: Pkl files, Starlark handlers, a `.gitignore`, and `IMPORT_REPORT.md`.

The pipeline runs without a network connection and without a running `gohomed` daemon. It is pure file I/O.
...
- **Does not migrate passwords or credentials.** HA stores password hashes in a format incompatible with gohome's authentication system. All users are migrated structurally (display name, role), but credentials are not transferred. After import you re-register each user's passkey via `gohome auth bootstrap <slug>`.
...
- **Does not translate Lovelace dashboards.** Lovelace dashboard YAML is detected and noted in the report, but translation is a separate future milestone. Rebuild dashboards using gohome's WYSIWYG dashboard editor.
- **Does not support HACS custom integrations.** Custom integrations outside gohome's driver set produce placeholder entries in the output. There is no automatic mapping.
...
```
gohome import-ha [<ha-dir>] -o <out-dir>
```
...
```
$ gohome import-ha ~/.homeassistant -o ./my-gohome
...
- [What transfers](what-transfers.md) — full mapping table from HA constructs to gohome targets, with confidence levels
