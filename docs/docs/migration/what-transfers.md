...
| HA construct | switchyard target | Confidence | Notes |
|---|---|---|---|
| `configuration.yaml` | `main.pkl` + `settings.pkl` | High | Core config maps directly. Global settings (unit system, time zone, currency, home coordinates) translate to the matching switchyard fields. |
...
| Device registry (`.storage/core.device_registry`) | Driver instances (diagnostic only) | Medium | switchyard's device model is driver-managed; no Pkl file is written. A NOTE in `IMPORT_REPORT.md` lists detected devices so you can verify them after drivers are running. |
...
| Lovelace dashboards | NOT migrated | N/A | Dashboards are detected and noted in `IMPORT_REPORT.md`. Translation is a separate future milestone. Rebuild dashboards in switchyard's WYSIWYG editor. |
| Users (`.storage/auth`) | `auth/users.pkl` | Medium | Display name and role (owner → admin, regular → member) translated. **Password hashes are not migrated** — re-register credentials via `switchyard auth bootstrap <slug>` post-import. |
...
The importer ships focused, field-by-field mappers for the integrations in switchyard's v1.0 driver set. For everything else it emits a labelled placeholder that preserves the original HA YAML so you can apply it manually once a driver is available.

| HA integration | switchyard driver | Confidence | Notes |
|---|---|---|---|
...
| HomeKit Controller (`homekit`) | `driver.homekit` | Medium | Pairing codes cannot be migrated. The driver instance is created with the bridge address, but re-pairing is required from the switchyard side. |
...
| Z-Wave JS (`zwave_js`) | `driver.zwave` | High | USB device path preserved. Network key (S0/S2) becomes a secret reference. Z-Wave JS must be running independently; switchyard connects to its WebSocket interface. |
...
| Generic webhook (`webhook`) | `driver.webhook` | Medium | Webhook IDs are preserved where possible, but the webhook base URL changes to your switchyard instance. Update any external services that send to the old HA webhook URL. |
...
| Other integrations | NOT supported | N/A | Integrations outside switchyard's v1.0 driver set produce `# FIXME(unmapped-integration)` placeholders. Items remain as skeleton config entries for manual wiring once a driver exists. |
...
```pkl
// FIXME(unmapped-integration): integration 'sonoff_lan' is outside switchyard's v1.0
// driver set. No mapper exists. Original HA configuration preserved below:
//
//   platform: sonoff_lan
//   username: myuser
//   password: !secret sonoff_password
new switchyard.imported.UnmappedIntegration {
...
| HA trigger platform | switchyard trigger | Notes |
|---|---|---|
...
| `homeassistant` (start/stop) | `EventTrigger` (`system_started` / `system_stopping`) | Direct mapping to switchyard system events. |
| `mqtt` | `EventTrigger` (`driver:mqtt`) | NOTE: requires the MQTT driver to be configured. |
| `webhook` | `EventTrigger` (`driver:webhook`) | NOTE: webhook URL changes to your switchyard instance. |
...
| HA action | switchyard action | Notes |
|---|---|---|
...
| `parallel: ...` | `# FIXME(unmapped-action)` | No parallel action in switchyard v1.0. |
